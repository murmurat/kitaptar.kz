...
func TestHandler_createBook(t *testing.T) {
	...
	for _, testCase := range testTable {
		t.Run(testCase.name, func(t *testing.T) {
			...
			request := httptest.NewRequest("POST", "/books", bytes.NewBufferString(testCase.inputBody))
...
func TestHandler_updateBook(t *testing.T) {
	...
	for _, testCase := range testTable {
		t.Run(testCase.name, func(t *testing.T) {
			...
			recorder := httptest.NewRecorder()
			request := httptest.NewRequest("PUT", fmt.Sprintf("/books/%s", testCase.inputBookID), bytes.NewBufferString(testCase.inputBody))
			request.Header.Set("Authorization", "Bearer token")
...
func TestHandler_getBookById(t *testing.T) {
	...
	testTable := []struct {
	...
	}{
		{
			...
			expectedStatusCode:   200,
			expectedResponseBody: `{"book_id":"00000000-0000-0000-0000-000000000000","author_id":"00000000-0000-0000-0000-000000000000","annotation":"test_annotation","name":"test_name","genre":"test_genre","image_path":"test_image_path","file_path_id":"00000000-0000-0000-0000-000000000000","CreatedAt":"0001-01-01T00:00:00Z"}`,
		},
		{
			name:    "Service error",
			inputID: "9634d0d0-ba9f-4516-9459-83eb58ebdb86",
	...
	for _, testCase := range testTable {
		t.Run(testCase.name, func(t *testing.T) {
			...
			recorder := httptest.NewRecorder()
			request := httptest.NewRequest("GET", strings.TrimSpace(fmt.Sprintf("/books/%s", testCase.inputID)), bytes.NewBufferString(testCase.inputID))
...
func TestHandler_deleteBook(t *testing.T) {
	...
	for _, testCase := range testTable {
		t.Run(testCase.name, func(t *testing.T) {
			...
			recorder := httptest.NewRecorder()
			request := httptest.NewRequest("DELETE", fmt.Sprintf("/books/%s", testCase.inputID), bytes.NewBufferString(testCase.inputID))
			request.Header.Set("Authorization", "Bearer token")
